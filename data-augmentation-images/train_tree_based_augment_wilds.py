...
def main(args):
    ...
    scale_list = [0.2, 0.6, 1.0]
    prob_list = [0.3, 0.6, 1.0]
        ...
        for i, augment_name in enumerate(task_list):
            for scale in scale_list:
                ...
                for prob in tmp_prob_list:
                    ...
                    if cur_val is None:
                        command = "python train_wilds.py --config configs/wild_config.json\
                                    --dataset {} --group_id {} \
                                    --augmentation_names {} \
                                    --augmentation_ratios {} \
                                    --augmentation_probs {} \
                                    --tree_idxes {} \
                                    --n_gpu 1 --device {} --epochs {} --batch_size {} --save_name {}".format( 
                                args.dataset, args.group_id,
                                ...
                                args.device,  args.epochs, args.batch_size, args.save_name,
                        )
                        os.system(command)
