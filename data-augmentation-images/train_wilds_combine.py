...
from data_augmentation.lisa import LISACutMix, LISAMixUp
...
def main(config, args):
    ...
    if args.train_bilevel or args.train_dro:
        ...
        for dataset_name in args.group_id:
            file_dir = os.path.join("../results/", f'wildcam_{dataset_name}_test.csv')
            result_df = pd.read_csv(file_dir, index_col=0)
            max_acc_row = result_df.loc[result_df['test_accuracy'].idxmax()]
            augmentation = max_acc_row['Augmentation']
            aug_and_ratio = augmentation.split('_')
            for i in range(0, len(aug_and_ratio), 2):
                best_augments[dataset_name].append(aug_and_ratio[i])
                best_ratios[dataset_name].append(float(aug_and_ratio[i+1]))
        ...
        for group_id in args.group_id:
            ...
            test_transform = pre_transforms + [ToTensor()] + post_transforms
        ...
        # set basic transform
        transform = pre_transforms + [ToTensor()] + post_transforms
        test_transform = pre_transforms + [ToTensor()] + post_transforms
    ...
    for run in range(args.runs):
        ...
        checkpoint_dir = "./saved/models/{}_{}_{}/".format(
            args.dataset, config["arch"]["type"], args.group_id) + ("_bilevel" if args.train_bilevel else "")\
...
if __name__ == '__main__':
    ...
    args.add_argument('--train_lisa_cutmix', action='store_true')
    args.add_argument('--train_lisa_mixup', action='store_true')
    args.add_argument('--lisa_alpha', type=float, default=0.5)
    ...
    args.add_argument('--train_dro', action="store_true")
