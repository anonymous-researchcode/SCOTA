...
from multitask_dataset import MultitaskDataset, MultitaskBatchSampler
from torch_geometric.loader import DataLoader
from tawt import get_task_weights_gradients_multi
...
def train_bilevel(model, device, loader, optimizer, group_weights, task_to_train_dataloaders,
                 weight_lr=1, collect_gradient_step=1):
    model.train()
    num_groups = len(group_weights)
    for step, batch in enumerate(loader):
        assert type(batch) == dict
        group_id = batch["task_name"][0]
        batch = batch["sample"]
        batch = batch.to(device)
        pred = model(
            batch.x, batch.edge_index, batch.edge_attr, batch.batch)
        ...
        loss_mat = criterion(pred.double(), y)
        optimizer.zero_grad()
        loss = torch.mean(loss_mat) * group_weights[group_id]*num_groups
        loss.backward()
    ...
    task_weights_gradients = get_task_weights_gradients_multi(
        model, task_to_train_dataloaders,
        criterion, device, collect_gradient_step
    )
    exp_ratio = torch.exp(- weight_lr * task_weights_gradients)
    new_task_weights = group_weights * exp_ratio
    group_weights = new_task_weights/torch.sum(new_task_weights)
    print(group_weights)
...
def main(args):
    ...
    for train_dataset in train_datasets:
        train_dataset.transform = augment_function
    '''
    Apply different transforms to differnt datasets
    '''
    task_to_train_datasets = {idx: train_datasets[idx] for idx in range(len(train_datasets))}
    task_to_train_dataloaders = {
        key: DataLoader(task_to_train_datasets[key], args.batch_size, shuffle=True, exclude_keys=["node_labels", "node_uncertainties", "graph_labels"]) \
        for key in task_to_train_datasets.keys()
    }
    train_dataset = MultitaskDataset(task_to_train_datasets)
    train_sampler = MultitaskBatchSampler(task_to_train_datasets, args.batch_size)
    train_loader = DataLoader(
        train_dataset,
        batch_sampler=train_sampler,
        exclude_keys=["node_labels", "node_uncertainties", "graph_labels"]
    )
    ...
    for run in range(1, args.runs+1):
        ...
        checkpoint_dir = "./saved_results/{}_group_{}_{}".format(args.gnn_type, group_str, augment_str) + \
            (f"_bilevel_{args.weight_lr}" if args.train_bilevel else "") + \
            (f"_dro_{args.weight_lr}" if args.train_dro else "") 
        if not os.path.exists(checkpoint_dir):
            os.mkdir(checkpoint_dir)

        # start training
        best_val_auroc = 0
        if args.train_bilevel or args.train_dro:
            group_weights = torch.ones(args.num_groups**2)/args.num_groups**2
            group_weights = group_weights.to(device)
        for epoch in range(1, args.epochs+1):
            if args.train_bilevel:
                group_weights = train_bilevel(model, device, train_loader, optimizer, group_weights, task_to_train_dataloaders,
                              args.weight_lr, args.collect_gradient_step)
            elif args.train_dro:
                group_weights = train_dro(model, device, train_loader, optimizer, group_weights, args.weight_lr)
            else:     
...
if __name__ == "__main__":
    ...
    parser.add_argument('--labeling_rate_threshold', type=float, default=0)
    ...
    ''' Bilevel training '''
    parser.add_argument('--train_bilevel', action='store_true')
    parser.add_argument('--weight_lr', type=float, default=1)
    parser.add_argument('--collect_gradient_step', type=int, default=1)
